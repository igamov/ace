<template>
  <div class="col-md-6">
    <div class="grid">
      <div class="grid-body">
        <div class="d-flex justify-content-between">
          <p class="card-title">Приоритеты</p>
          <div class="btn-group">
      ...
      <table class="table table-hover">
        ...
        <tbody>
        <tr v-for="(item, index) in priorities" v-bind:key="item.id">
          <td>{{index + 1}}</td>
          <td>{{item.title}}</td>
          <td>{{item.color}}</td>
          <td class="actions">
    ...
    <div class="modal fade" id="delete_priority" tabindex="-1" role="dialog">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="add_priority">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <form v-on:submit.prevent="editmode ? updateItem() : createItem()">
            ...
            <div class="modal-body">
              <div class="form-group">
                <label for="priority_title">Title</label>
              ...
              <div class="form-group">
                <label for="priority_color">Color</label>
                <input type="text" v-model="form.color"
                       :class="'form-control' + (errors.color ? ' is-invalid' : '')" id="priority_color"
                       placeholder="Color">
  ...
  export default {
    ...
    methods: {
      ...
      createItem() {
        this.loading = false;
        var vm = this;
        axios.post(route('priority.store'), this.form)
          .then(function (response) {
            ...
            vm.priorities.push(response.data);
      ...
      updateItem(){
        var vm = this;
        axios.put(route('priority.update', [this.form.id]), this.form)
          .then((response) => {
            pnotify.alert({
              title: 'Success',
            ...
            vm.priorities.splice(vm.form.index, 1, response.data);
            $('#add_priority').modal('hide');
            vm.clearForm();
          })
          .catch((error) => {
      ...
      setDelete(){
        ...
        axios.delete(route('priority.destroy', [this.form.id]))
          .then(() => {
            ...
            vm.priorities.splice(vm.form.index, 1);
            $('#delete_priority').modal('hide');
          })
          .catch((error) => {
            if (error.response) {
              this.errors = error.response.data.errors;
