  ...
  <div class="col-md-6" v-if="!loading">
    <div class="grid">
      ...
      <table class="table table-hover">
        ...
        <tbody>
        <tr v-for="(item, index) in statuses" v-bind:key="item.id">
          <td>{{index + 1}}</td>
          <td>{{item.title}}</td>
          <td class="actions">
    ...
    <div class="modal fade" id="delete_status" tabindex="-1" role="dialog">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="add_status">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
  ...
  export default {
    ...
    methods: {
      ...
      createItem() {
        this.loading = false;
        var vm = this;
        axios.post(route('status.store'), this.form)
          .then(function (response) {
            ...
            vm.statuses.push(response.data);
      ...
      updateItem(){
        var vm = this;
        axios.put(route('status.update', [this.form.id]), this.form)
          .then((response) => {
            pnotify.alert({
              title: 'Success',
            ...
            vm.statuses.splice(vm.form.index, 1, response.data);
            $('#add_status').modal('hide');
            vm.clearForm();
          })
          .catch((error) => {
      ...
      setDelete(){
        var vm = this;
        axios.delete(route('status.destroy', [this.form.id]))
          .then(() => {
            pnotify.alert({
              title: 'Success',
            ...
            vm.statuses.splice(vm.form.index, 1);
